...
async function fetchTimeIndices() {
    console.log('Ermittle Referenzdatum und Zeitachse (time) für die Wolkendaten...');
    const das = await fetchText(`${BASE_URL}.das`);
    const timeBlockMatch = das.match(/\btime\s*\{[^}]*units\s+"Hour since ([^"]+)"/);
...
async function fetchSingleStep(threddsIdx, stepNumber, totalSteps) {
    ...
    const dataBlock = parts[1].split(`${VAR}.time`)[0];
...
const cloudData = (() => {
    ...
    const ptsPerStep = cloudMetadata.nx * cloudMetadata.ny;
    return cloudMetadata.steps.map((s, idx) => {
        const sub = int16.subarray(idx * ptsPerStep, (idx + 1) * ptsPerStep);
        return {
            hour: s.hour,
            timestamp: s.timestamp,
            timeMs: s.timeMs,
            header: { nx: cloudMetadata.nx, ny: cloudMetadata.ny, dx: cloudMetadata.dx, dy: cloudMetadata.dy, la1: cloudMetadata.la1, lo1: cloudMetadata.lo1 },
            scale: cloudMetadata.scale,
            data: sub
        };
    });
})();
`;
        ...
        console.log('ERFOLG: cloud_data.js geschrieben!');
    } catch (e) {
